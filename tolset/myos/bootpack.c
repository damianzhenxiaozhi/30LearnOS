...
void HariMain(void)
{
	...
	init_palette();
	shtctl = sheet_init(memman, binfo->vram, binfo->scrnx, binfo->scrny);
	sht_back = sheet_alloc(shtctl);
	sht_mouse = sheet_alloc(shtctl);
	buf_back = (unsigned char *)memman_alloc_4k(memman, binfo->scrnx * binfo->scrny);
	sheet_setbuf(sht_back, buf_back, binfo->scrnx, binfo->scrny, -1);
	sheet_setbuf(sht_mouse, buf_mouse, 16, 16, -1);
	...
	for (;;) {
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				io_sti();
				sprintf(s, "%02X", i);
				boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 16, 31, 31);
				...
				sprintf(s, "(%3d, %3d)", mx, my);
				boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 0, 10 * 8 - 1, 15);
				putfonts8_asc(buf_back, binfo->scrnx, 0, 0, COL8_FFFFFF, s);
				sheet_slide(shtctl, sht_mouse, mx, my);
			}
		}
